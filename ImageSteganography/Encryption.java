...
public class Encryption extends JFrame implements ActionListener {
    ...
    public Encryption() {
        ...
        embed.setBackground(Color.pink);
    ...
    private void embedMessage() {
        ...
        if (mess.equals("")) {
            JOptionPane.showMessageDialog(this, "Please Enter a Text First");
            return;
        } else if (sourceImage.getHeight() == 0) {
            JOptionPane.showMessageDialog(this, "Please Select a valid image first!");
            return;
        }
    ...
    private void embedMessage(BufferedImage img, String mess) {
        ...
        embedInteger(img, messageLength);
    ...
    private void embedInteger(BufferedImage img, int messagelength) {
        int maxX = img.getWidth(), maxY = img.getHeight(),
                startX = 0, startY =0, count = 0;
        for (int i = startX; i < maxX && count < 32; i++) {
            for (int j = startY; j < maxY && count < 32; j++) {
                int rgb = img.getRGB(i, j), bit = getBitValue(messagelength, count);
    ...
    private int getBitValue(int messageLength, int location) {
        int v = messageLength & (int) Math.round(Math.pow(2, location));
        return v == 0 ? 0 : 1;
    }
